...
{
	return (ADC_ConvertedValue[Channel] + ADC_ConvertedValue[Channel + 4] + ADC_ConvertedValue[Channel + 8] + ADC_ConvertedValue[Channel + 12] + ADC_ConvertedValue[Channel + 16])/5;
}

float UADC_GetValue(uint8_t Channel)
{
	float value = 0.0;
	if(Channel == Temp_On_Chip)
	{
		value = UADC_Temp_On_Chip.Value;
	}
	else if(Channel == Temp_LM35)
	{
		value = UADC_Temp_LM35.Value;
	}
	else if(Channel == iMotor)
	{
